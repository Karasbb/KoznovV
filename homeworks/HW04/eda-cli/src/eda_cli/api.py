...
from .core import compute_quality_flags, missing_table, summarize_dataset, top_categories, correlation_matrix, flatten_summary_for_print
...
def quality_flags_from_csv(file: UploadFile = File(...)):
    ...
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    flags = compute_quality_flags(summary, missing_df, has_constant_columns, as_high_cardinality_categoricals, too_few_rows, too_many_columns)  
    return {"flags": flags}
